jQuery(document).ready(function () {
  ...
  if (currentUri.includes(loginText)) {
    ...
    jQuery(".mo-openid-app-icons").append(
    ...
    var ajaxUrl =
      window.bigNinjaVoteUrl.ajaxUrl + "/v1/wp_update_term_users_status";
    var ajaxUrl1 =
      window.bigNinjaVoteUrl.ajaxUrl + "/v1/wp_update_option_redirect";
    ...
    if (currentUri.includes(loginText) && currentUri.includes(actionRegister)) {
      ...
      jQuery("#registerform").append(
        '<div class="login-form-footer">Registration confirmation will be emailed to you.</div>'
      );
      jQuery("#registerform").append(
        '<div id="div-header"><span class="you-dont-have-account">You have an account?</span><a id="button-register" class="ant-btn" href="' +
          window.bigNinjaVoteUrl.siteURL +
          '/wp-login.php">Login</a></div>'
      );
    ...
    if (
      ...
      jQuery("#lostpasswordform").append(
        '<div class="login-form-footer">New password will be emailed to you.</div>'
      );
      jQuery("#lostpasswordform").append(
        '<div id="div-header"><a id="button-register" class="ant-btn lostpass-button-login" href="' +
          window.bigNinjaVoteUrl.siteURL +
          '/wp-login.php">Login</a><a id="button-register" class="ant-btn" href="' +
          window.bigNinjaVoteUrl.siteURL +
          '/wp-login.php?action=register">Register</a></div>'
      );
    }
    if (
      currentUri.includes(loginText) &&
      ...
      jQuery("#resetpassform").append(
        '<div class="login-form-footer">The password should be at least twelve characters long. To make it stronger, use upper and lower case letters, numbers, and symbols like ! " ? $ % ^ &amp; ).</div>'
      );
      jQuery("#resetpassform").append(
        '<div id="div-header"><a id="button-register" class="ant-btn lostpass-button-login" href="' +
          window.bigNinjaVoteUrl.siteURL +
    ...
    if (
      currentUri.includes(loginText) &&
      currentUri.includes(actionResetPass)
    ) {
...
function showLoginForm() {
  ...
  document
    .getElementsByClassName("user-pass-wrap")[0]
    .classList.add("user-pass-wrap");
  document.getElementsByClassName("submit")[0].classList.add("email-active");
  document
    .getElementsByClassName("forgetmenot")[0]
    .classList.add("email-active");
  document.getElementsByClassName("wp-pwd")[0].classList.add("email-active");
  document.getElementById("user_pass").disabled = false;
  document.getElementsByClassName("mo-openid-app-icons")[0].style.display =
    "none";
  document.getElementsByClassName("login-form-footer")[0].style.display =
    "none";
  ...
  a.setAttribute(
    "href",
    window.bigNinjaVoteUrl.siteURL + "/wp-login.php?action=lostpassword"
  );
  ...
  register.setAttribute(
    "href",
    window.bigNinjaVoteUrl.siteURL + "/wp-login.php?action=register"
  );
  document.getElementById("login-header-title").innerHTML =
    '<a class="login-back" href="' +
    window.bigNinjaVoteUrl.siteURL +
    '/wp-login.php"><i aria-label="icon: arrow-left" class="anticon anticon-arrow-left"><svg viewBox="64 64 896 896" data-icon="arrow-left" width="18px" height="18px" fill="currentColor" aria-hidden="true" focusable="false" ><path d="M872 474H286.9l350.2-304c5.6-4.9 2.2-14-5.2-14h-88.5c-3.9 0-7.6 1.4-10.5 3.9L155 487.8a31.96 31.96 0 0 0 0 48.3L535.1 866c1.5 1.3 3.3 2 5.2 2h91.5c7.4 0 10.8-9.2 5.2-14L286.9 550H872c4.4 0 8-3.6 8-8v-60c0-4.4-3.6-8-8-8z"></path></svg></i></a><span>Login with Email</span>';
}
