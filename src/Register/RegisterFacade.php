...
class RegisterFacade {
	...
	protected function __construct() {
		...
		$is_prod = ! defined( 'FB_IS_DEVELOPMENT' ) || FB_IS_DEVELOPMENT !== true;
		if ( $is_prod && class_exists( '\Feedbo\Register\RegisterProd' ) ) {
			\Feedbo\Register\RegisterProd::get_instance();
		} elseif ( ! $is_prod && class_exists( '\Feedbo\Register\RegisterDev' ) ) {
			\Feedbo\Register\RegisterDev::get_instance();
		}
	}
	...
	public function register_all_assets() {
		wp_register_style(
			ScriptName::STYLE_FEEDBO,
			FB_PLUGIN_URL . 'assets/dist/main.css',
			array(),
			FB_PLUGIN_VERSION
		);
	}
	...
	public function use_mo_file_for_script_translations( $json_translations, $file, $handle, $domain ) {
		...
		if ( 'feedbo' !== $domain || ! in_array( $handle, $all_handles, true ) ) {
			return $json_translations;
		}

		$translations = get_translations_for_domain( 'feedbo' );
		$messages     = array(
			'' => array(
				'domain' => 'messages',
			),
