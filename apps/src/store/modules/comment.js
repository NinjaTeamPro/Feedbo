...
export const mutations = {
  ...
  UPDATE_LIKE(state, commentUpdate) {
    ...
    let userLikeIds =
      state.comments[index].users_like != null
        ? state.comments[index].users_like.split(" , ")
        : [];
    if (commentUpdate.checkLike == false) {
      state.comments[index].userslike_length =
        state.comments[index].userslike_length + 1;
      userLikeIds.push(commentUpdate.userLike);
    } else {
      state.comments[index].userslike_length =
        state.comments[index].userslike_length - 1;
      let likeIndex =
        commentUpdate.checkAnonymous == true
          ? userLikeIds.lastIndexOf(commentUpdate.userLike)
          : userLikeIds.indexOf(commentUpdate.userLike);
      userLikeIds.splice(likeIndex, 1);
    }
    state.comments[index].users_like =
      state.comments[index].userslike_length == 1
        ? commentUpdate.userLike
        : userLikeIds.join(" , ");
  },
};
export const actions = {
    ...
    grecaptcha.ready(function() {
      grecaptcha
        ...
        .then(function(token) {
          ...
          return ApiService.addComment(comment, state.commentImgUrl)
            .then((response) => {
              commit("SET_GENERNAL", {
  ...
  deleteComment({ commit }, data) {
    ...
    return ApiService.deleteComment(data)
      .then((response) => {
        commit("SET_GENERNAL", {
          comments: response.data.comments,
        });
