  ...
  <div>
          ...
          <AList :data-source="post.listPost">
            ...
            >
                ...
                >
                  ...
                  <span class="post-title-status">
                    <ABadge 
                      ...
                      :number-style="{ backgroundColor: getStatusColor(item.post_status) }" 
                    />
                  </span>
              ...
              <AButton
                ...
                class="button-vote"
                :style="[checkAnonymousVote(item) ? {borderLeft: 'solid 4px '+category.theme.accent} : {}]"
              ...
              <div v-else>
                ...
                <AButton
                  ...
                  :style="[checkAnonymousDownVote(item) ? {borderLeft: 'solid 4px '+category.theme.accent} : {}]"
    ...
    >
      <ASkeleton :loading="post.loadingRoadmap" active>
        <template v-if="post.status && category.board.features.indexOf('roadmap') > -1">
          <ATimeline>
            <div
...
export default {
    ...
    watch: {
        listPost: function (newValue) {
            if (newValue.length > 0) {
                if (this.$route.hash !== '' && this.$route.hash.substring(0,1) === '#') {
    ...
    mounted() {
        ...
        listElm.addEventListener('scroll', e => {
            if(this.mobileCheck()) {
                ...
                const headerElement = document.querySelector('.big-ninja-feedbo .head-container.board-header');
                const logoElement = document.querySelector('.big-ninja-feedbo .board-home-link');
                const accountElemnt = document.querySelector('.big-ninja-feedbo .nav-container.board-navbar');
                const boardElemnt = document.querySelector('.big-ninja-feedbo .board-content');
                if(scrollTop === 0) {
                    headerElement.style.display = 'block';
                    logoElement.style.display = 'block';
                    accountElemnt.style.display = 'block';
                    boardElemnt.style.setProperty('padding', 'unset');
                }
                else if(scrollTop > 0 && scrollTop < scrollHeight) {
                    headerElement.style.display = 'none';
                    logoElement.style.display = 'none';
                    accountElemnt.style.display = 'none';
    ...
    methods: {
        fetchData() {
            ...
            let loadIndex = this.post.postIndexLoad + 10 > maxLength ? maxLength : this.post.postIndexLoad + 10;
        ...
        handleClick (post) {
            if (this.user.userAnonymous == true)
            {
                ...
                {this.$message.error('Anonymous does not vote on this board. Please login and continue.');}
                else
                {
                    ...
                    this.$store.commit('post/UPDATE_VOTE',{post, checkVote, checkDownVote, checkAnonymous, userVote});
        ...
        downVoteClick (post){
            ...
            {
                if (this.checkAllowAnonymous() == false)
                {this.$message.error('Anonymous does not downvote on this board. Please login and continue.');}
                else
                {
                    let checkVote = this.checkAnonymousVote(post);
                    let checkDownVote = this.checkAnonymousDownVote(post);
                    ...
                    this.$store.commit('post/UPDATE_DOWN_VOTE',{post, checkVote, checkDownVote, checkAnonymous, userVote});
